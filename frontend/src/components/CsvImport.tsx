...
interface Props {
  onLoaded: (points: Point[], fileName: string, ignored: number, error?: string) => void;
...
export function CsvImport({ onLoaded, fileName, pointCount, ignored, error }: Props) {
  const handle = (file: File) => {
    const reader = new FileReader();
    reader.onload = () => {
      const text = String(reader.result ?? '');
      const res = parseCsvPoints(text);
      onLoaded(res.points, file.name, res.ignored, res.error);
    };
